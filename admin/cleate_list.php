<?php
  ...
  $currencies = new currencies();
	...
	switch ($HTTP_GET_VARS['action']){
		...
		for($i=0;$i < $list_cnt;$i++){
			$res_cnt=tep_db_query("select count(*) as data_cnt from set_oroshi_datas where parent_id='".$cPath."' AND oroshi_name='".$oroshi_name[$i]."' ORDER BY list_id ASC " );
			$col_cnt[]=tep_db_fetch_array($res_cnt);
			
			$res_id=tep_db_query("select * from set_oroshi_datas where parent_id='".$cPath."' AND oroshi_name='".$oroshi_name[$i]."' ORDER BY list_id ASC " );
			$col_id[]=tep_db_fetch_array($res_id);
		}
		for($i=0;$i < $list_cnt;$i++){
			if($setdata[$i] != "" && $col_cnt[$i]['data_cnt'] != 20){
				tep_db_query("insert into set_oroshi_datas (oroshi_name,parent_id,datas,set_date) values ('".$oroshi_name[$i]."','".$cPath."','".$setdata[$i]."','".$date."')");
			}else if($setdata[$i] != ""){
...
<table border="0"class="list">
			...
			<tr>
				<?php
					for($i=0;$i < $cnt;$i++){
						echo "<td><textarea rows='5' cols='30' name='set_list[]' ></textarea></td>";
						echo "<input type='hidden' value='".$o_name[$i]."' name='oroshi_name[]'>";					
					}
