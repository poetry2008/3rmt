...
$res=tep_db_query("select oroshi_name from set_oroshi_names where parent_id='".$cPath."' ORDER BY oroshi_id ASC");
$cnt=0;
...
for($i=0;$i < $count[0];$i++){
	...
	echo "<td><select name='select_data[]' onChange='menuLink(this.options[this.selectedIndex].value,".$i.",".$count[0].")'>";
...
<tr>
<td>
<input type="button" value="登録"　name="b1" onClick="list_submit()" >
</td>
<td>
<input type="button" value="選択リセット"　name="b2" onClick="select_reset(<?php echo $count[0]; ?>,<?php echo $cnt3; ?>)" >
</td>
</tr>
</form>
</table>
