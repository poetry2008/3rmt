    ...
    if($check_customer['customers_guest_chk'] == '0') {
      $random_str = md5(time().$check_customer['customers_id'].$_POST['email_address']); 
        ...
        if (tep_db_num_rows($exists_password_raw)) {
          tep_db_query("update `customers_password_info` set `customers_email` = '".$_POST['email_address']."', `customers_ip` = '".$_SERVER["REMOTE_ADDR"]."', `random_num` = '".$random_str."', `created_at` = '".date('Y-m-d H:i:s',time())."' where `customers_id` = '".$check_customer['customers_id']."'"); 
        } else {
        ...
        <?php echo tep_draw_form('password_forgotten', tep_href_link(FILENAME_PASSWORD_FORGOTTEN, 'action=process', 'SSL')); ?>
          <table border="0" width="100%" cellspacing="0" cellpadding="0" summary="table" class="product_info_box">
            <tr>
              <td colspan="2"></td>
            <tr>
              <td align="left" class="main"><?php echo ENTRY_FORGOTTEN_EMAIL_ADDRESS; // 2003.03.06 nagata Edit Japanese osCommerce ?></td>
              ...
              <td colspan="2"><br>
                <table border="0" cellpadding="0" cellspacing="0" width="100%" summary="table">
                  <tr>
                    ...
                    <td align="right" valign="top"><?php echo tep_image_submit('button_continue02.gif', IMAGE_BUTTON_CONTINUE); ?></td>
                  </tr>
                </table>
              </td>
            </tr>
