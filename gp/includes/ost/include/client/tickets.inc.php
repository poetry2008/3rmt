    ...
    <tr><td>
     <table border="0" cellspacing="1" cellpadding="2" class="tickets" align="center" width="100%">
        <tr>
          <th width="60" nowrap>
                <a href="<?php echo
                tep_href_link('view.php','sort=ID&order='.$negorder.$qstr,'SSL');?>" title="番号順に表示 <?=$_negorder?>">問合番号</a></th>
          <th width="70">
                <a href="<?php echo
                tep_href_link('view.php','sort=date&order='.$negorder.$qstr,'SSL');?>" title="作成日順に表示 <?=$_negorder?>">作成日</a></th>
            <th>件名</th>
        </tr>
        <?
        $class = "row1";
        ...
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            $_closed = 'クローズ';
                  ?>
                <td>&nbsp;<a href="<?php echo
                tep_href_link('view.php','id='.$row['ticketID'],'SSL');?>"><?=$subject?></a>
