...
(function($) {
  $.fn.adGallery = function(options) {
    var defaults = { loader_image: 'images/ad-gallery/loader.gif',
                     start_at_index: 0,
                     thumb_opacity: 0.7,
  ...
  AdGallery.prototype = {
    ...
    init: function(wrapper, settings) {
      ...
      this.nav_display_height = this.nav.width();
      this.images = [];
    ...
    initNextAndPrev: function() {
      this.next_link = $('<div class="ad-forward"></div>');
      this.prev_link = $('<div class="ad-back"></div>');

      this.nav.append(this.next_link);
      this.nav.prepend(this.prev_link);
      var context = this;
    ...
    initBackAndForward: function() {
      ...
      $(this.scroll_back).add(this.scroll_forward).click(
        function() {
          ...
          var width = context.nav_display_height - 50;
        ...
        function() {
          ...
          context.thumbs_scroll_interval = setInterval(
            function() {
              var scrollTop = context.thumbs_wrapper.scrollTop() + 1;
              if(direction == 'top') {
                scrollTop = context.thumbs_wrapper.scrollTop() - 1;
              }
              context.thumbs_wrapper.scrollTop(scrollTop);
            },
            10
          );
          $(this).css('opacity', 1);
    ...
    _showWhenLoaded: function(index, callback) {
      if(this.images[index]) {
        ...
        img_link.attr('href', image.image.replace('m.', 'l.'));
        //img_link.attr('title', "title");
        img_link.addClass('jqzoom');
        ...
        /*if ($.browser.msie){
          img_container.css({width: image_width + 3 +'px', height: image_height + 2 +'px'});
        } else {
          img_container.css({width: image_width +'px', height: image_height + 1 +'px'});
        }*/
    ...
    highLightThumb: function(thumb) {
      ...
      var left = thumb[0].parentNode.offsetTop - 155; // 这里
      left -= (this.nav_display_height / 2) - (thumb[0].offsetHeight / 2);
      if(this.settings.animated_scroll) {
        this.thumbs_wrapper.animate({scrollTop: left +'px'});
      } else {
        this.thumbs_wrapper.scrollTop(left);
      }
    }
  };
})(jQuery);
