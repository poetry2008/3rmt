...
// Quantity box or information as an input box or text
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      echo '    <td align="center" style=" background:#dbd6d6;padding-left:10px;padding-right:20px;">';
      echo '<table>'; 
      echo '<tr>'; 
      echo '<td>'; 
      ...
      echo '    <td align="center" class ="main" style=" background:#dbd6d6;padding-left:10px;padding-right:20px;">' . $products[$i]['quantity'] . '個';
    ...
    //add products image 
    echo '<td align="center" class="main" style=" background:#dbd6d6;padding-left:10px;padding-right:20px;">';
...
// Product name, with or without link
    ...
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      if ($cart_pro_status['products_status'] == 0 || $cart_pro_status['products_status'] == 3) {
        ...
        echo '    <td class="main" style=" background:#dbd6d6">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?$products[$i]['model'] . '<br>':'').'<b>' . $products[$i]['name'] . '</b>';
      } else {
        echo '    <td class="main" style=" background:#dbd6d6">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . tep_get_prid($products[$i]['id'])) . '">' . $products[$i]['model'] . '</a><br>':'').'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . tep_get_prid($products[$i]['id'])) . '"><b>' . $products[$i]['name'] . '</b></a>';
      }
    } else {
      echo '    <td class="main" style=" background:#dbd6d6">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . tep_get_prid($products[$i]['id'])) . '">' . $products[$i]['model'] . '</a><br>':'').'<b>' . $products[$i]['name'] . '</b>';
    }
...
// Tax (not in shopping cart, tax rate may be unknown)
    if (!strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td align="center" class="main" style=" background:#dbd6d6">' . number_format($products[$i]['tax'], TAX_DECIMAL_PLACES) . '%</td>' . "\n";
    }
...
// Product price  
    if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
      ...
      // edit total 
      echo '    <td align="right" class="main" style="background:#dbd6d6"><span id="pri_'.$products[$i]['id'] .'">'.  $currencies->display_price($products[$i]['price'], tep_get_tax_rate($products[$i]['tax_class_id']), $products[$i]['quantity']) . '</span>';
    } else {
      echo '    <td align="right" class="main" style=" background:#dbd6d6"><b>' . $currencies->display_price($products[$i]['price'], $products[$i]['tax'], $products[$i]['quantity']) . '</b>';
    }
