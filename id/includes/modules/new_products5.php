...
*/
  ...
  if ( (!isset($new_products_category_id)) || ($new_products_category_id == '0') ) {
    ...
    $new_products_query = tep_db_query("
        ...
        from " . TABLE_PRODUCTS . " p, ".TABLE_PRODUCTS_PRODUCTS_DESCRIPTION." pd 
    ...
    if (!empty($has_c_arr)) {
      $new_products_query = tep_db_query("
          ...
          where p.products_id = p2c.products_id 
            ...
            and c.categories_id in (" .implode(',', $has_c_arr) . ") and p.products_id = pd.products_id ".(BOX_NEW_PRODUCTS_DAY_LIMIT ? ( " and p.products_date_added > '" . date('Y-m-d H:i:s', time()-(BOX_NEW_PRODUCTS_DAY_LIMIT*86400)) . "'" ) : '')." order by pd.site_id DESC) c where site_id = '".SITE_ID."' or site_id = '0' group by products_id having c.products_status != '0' and c.products_status != '3' order by products_date_added desc limit " . MAX_DISPLAY_NEW_PRODUCTS);
    } else {
      $new_products_query = tep_db_query("
          select * from (select distinct p.products_id, 
                          p.products_quantity, 
      ...
      <table width="670" border="0" cellspacing="0" cellpadding="0" style="margin: 10px;">
        ...
        <tr>
          <td class="main" style="padding-left:5px; ">
            <p class="main">
              <?php echo '残り&nbsp;' . number_format(tep_show_quantity($new_products['products_quantity'])) . '個以上'; ?>
              &nbsp;&nbsp;&nbsp;
<?php
