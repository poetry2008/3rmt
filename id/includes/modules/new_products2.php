    ...
    if (!empty($has_c_arr)) {
      $new_products_query = tep_db_query("
          ...
          where p.products_id = p2c.products_id 
            ...
            and c.categories_id in (" . implode(",", $has_c_arr) . ") ".(BOX_NEW_PRODUCTS_DAY_LIMIT ? ( " and p.products_date_added > '" . date('Y-m-d H:i:s', time()-(BOX_NEW_PRODUCTS_DAY_LIMIT*86400)) . "'" ) : '')." order by pd.site_id DESC) c where site_id = '".SITE_ID."' or site_id = '0' group by products_id having c.products_status != '0' and c.products_status != '3' order by products_date_added desc limit " . MAX_DISPLAY_NEW_PRODUCTS);
    } else {
      $new_products_query = tep_db_query("
          select * from (select distinct p.products_id, 
                          p.products_real_quantity + p.products_virtual_quantity as products_quantity,
      ...
      <table width="670" border="0" cellspacing="0" cellpadding="0" style="margin: 10px;">
        <tr>
          ...
          <td valign="top" style="padding-left:5px; ">
            <h3 class="main">
              <?php echo '<a class="arrow_2" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $new_products['products_id']) . '">'.$new_products['products_name'].'</a>'; ?><br>
            </h3>
          </td>
          <td align="right">
