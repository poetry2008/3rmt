...
<div class="index_reviews05">
  ...
  $random_reviews = tep_reviews_random_select($random_reviews_str, 1); 
...
// display random review box
    $review_query = tep_db_query("select substring(reviews_text, 1, 50) as reviews_text from " . TABLE_REVIEWS_DESCRIPTION . " where reviews_id = '" .  $ran_key . "' and languages_id = '" . $languages_id . "'");
    $review = tep_db_fetch_array($review_query);

    $review = htmlspecialchars($review['reviews_text']);
    //$review = tep_break_string($review, 15, '-<br>');

    echo '<p class="reviews_top"><a href="' .  tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' .  $ran_value['products_id'] . '&reviews_id=' . $ran_key) . '" class="reviews_img">' . tep_image(DIR_WS_IMAGES .  $ran_value['products_image'], $ran_value['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br>'.  tep_image(DIR_WS_IMAGES . 'stars_' . $ran_value['reviews_rating'] . '.gif' , sprintf(BOX_REVIEWS_TEXT_OF_5_STARS, $ran_value['reviews_rating']), 88, 16) . "\n".'</p> <p class="reviews_bottom"><a href="' .  tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' .  $ran_value['products_id'] . '&reviews_id=' . $ran_key) . '">' . $review . ' ...</a></p>'; 
   } 
