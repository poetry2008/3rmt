...
while ($manufacturer = tep_db_fetch_array($manufacturer_query)){
  ...
  if (tep_db_num_rows($products_query)) {
    ...
    while($products = tep_db_fetch_array($products_query)) {
      ...
      echo '<td align="center" valign="top" class="smallText" width="30%"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '">'.tep_image2(DIR_WS_IMAGES.$products['products_image'],$products['products_name'],SMALL_IMAGE_WIDTH,SMALL_IMAGE_HEIGHT,'class="image_border"').'<br>' .$products['products_name'] . '</a><br>'.$products_price.'<!-- '.strip_tags(substr($products['products_description'],0,50)).' --></td>'."\n";
    }
    echo '        </tr>' . "\n";
    echo '      </table>' . "\n";
    echo '    </td>' . "\n";
