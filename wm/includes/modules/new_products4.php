    ...
    if ($subcategories) {
      ...
      $new_products_query = tep_db_query("
          ...
          from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " . TABLE_CATEGORIES . " c, ".TABLE_PRODUCTS_DESCRIPTION." pd where p.products_id = p2c.products_id 
            ...
            and p.products_id = pd.products_id 
            ".(BOX_NEW_PRODUCTS_DAY_LIMIT ? ( " and p.products_date_added > '" . date('Y-m-d H:i:s', time()-(BOX_NEW_PRODUCTS_DAY_LIMIT*86400)) . "'" ) : '')." 
          order by pd.site_id DESC) c where site_id = '".SITE_ID."' or site_id = '0' group by products_id having c.products_status != '0' order by products_date_added desc 
          limit " . MAX_DISPLAY_NEW_PRODUCTS);
      $num_products       = tep_db_num_rows($new_products_query);
    }
  }
...
<h3 class="pageHeading new_products03"><?php echo $new_c_name; ?>の新着商品</h3>
<div class="comment comment_products03">
<table width="100%" cellspacing="0" cellpadding="0" border="0" style="background: none repeat scroll 0% 0% rgb(242, 242, 242); padding-top: 10px;">
<?php
...
?>
  <tr>
    <td>
      <table cellspacing="0" cellpadding="0" border="0" class="product_listing_content">
        <tr>
          <td width="<?php echo SMALL_IMAGE_WIDTH;?>" rowspan="2" style="padding-right:5px; " align="center">
          ...
          <td class="main" valign="top" style="padding-left:5px;">
            <p class="main">
