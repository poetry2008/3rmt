...
<?php
  ...
  for($i=0; $i<sizeof($products); $i++) {
	...
	echo '      <tr>' . "\n";
	echo '        <td width="25" valign="top" class="main">' . mb_convert_encoding($products[$i]['quantity'] . '個','UTF-8','EUC-JP') . '</td>' . "\n";
...
?>      
  ...
  <tr>
	...
	<?php echo mb_convert_encoding('個数：<strong>' . number_format(sizeof($products)).'個','UTF-8','EUC-JP') . '</strong>'; ?>&nbsp;
	<?php echo mb_convert_encoding('合計：<strong style="font-size:16px; color:red;">' . $currencies->format($cart->show_total()),'UTF-8','EUC-JP') . '</strong>'; ?>&nbsp;
