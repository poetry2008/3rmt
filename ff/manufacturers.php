...
<?php 
  $mcaid_arr = tep_other_get_categories_id_by_parent_id(FF_CID);
  if (empty($mcaid_arr)) {
...
while ($manufacturer = tep_db_fetch_array($manufacturer_query)){
  ...
  $products_query = tep_db_query("
      select * from (select p.products_date_added, pd.site_id, pd.products_status, p.products_id, p.products_image, p.products_tax_class_id, p.products_bflag, p.products_price, p.products_price_offset, p.products_small_sum from " . TABLE_PRODUCTS . " p, ".TABLE_PRODUCTS_DESCRIPTION." pd 
