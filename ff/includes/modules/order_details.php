...
// Product name, with or without link
    ...
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      if ($cart_pro_status['products_status'] == 0 || $cart_pro_status['products_status'] == 3) {
        ...
        echo '    <td class="main" style=" background:#FFFFFF">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?$products[$i]['model'] . '<br>':'').'<div class="product_open"><b>' . $products[$i]['name'] . '</b>';
      } else {
        echo '    <td class="main" style=" background:#FFFFFF">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' .  tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .  (int)$products[$i]['id']) . '">' . $products[$i]['model'] .  '</a><br>':'').'<div class="product_open"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . (int)$products[$i]['id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
      }
    } else {
      echo '    <td class="main" style=" background:#FFFFFF">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' .  tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .  (int)$products[$i]['id']) . '">' . $products[$i]['model'] .  '</a><br>':'').'<div class="product_open"><b>' . $products[$i]['name'] . '</b>';
    }
...
// Display marker if stock quantity insufficient
    ...
    if($products_error == true && $stock_check == ''){
      ...
      echo '<span class="markProductOutOfStock"><a style="color:#CC0033" href="'.tep_href_link('open.php', 'products_name='.urlencode($products[$i]['name'])).'">' . STOCK_MARK_PRODUCT_OUT_OF_STOCK . '</a></span>';
    }
    echo '</div>';
