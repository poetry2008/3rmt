...
<?php
    ...
    for($i=0, $n=sizeof($products_new_array); $i<$n; $i++) {
      if (tep_get_special_price($products_new_array[$i]['price'], $products_new_array[$i]['price_offset'], $products_new_array[$i]['small_sum'])) {
        $products_price = '<s>' . $currencies->display_price(tep_get_price($products_new_array[$i]['price'], $products_new_array[$i]['price_offset'], $products_new_array[$i]['small_sum'])) . '</s>&nbsp;&nbsp;<span class="productSpecialPrice">' . $currencies->display_price(tep_get_special_price($products_new_array[$i]['price'], $products_new_array[$i]['price_offset'], $products_new_array[$i]['small_sum'])) . '</span>&nbsp;';
      } else {
        $products_price = $currencies->display_price(tep_get_price($products_new_array[$i]['price'], $products_new_array[$i]['price_offset'], $products_new_array[$i]['small_sum']));
      }
