...
// class methods
    ...
    function confirmation() {
      ...
      if (!empty($HTTP_POST_VARS['postal_money_order_fee'])) {
        //$s_message = $s_result ? (MODULE_PAYMENT_POSTALMONEY_ORDER_TEXT_FEE . '&nbsp;' .  $currencies->format($HTTP_POST_VARS['postal_money_order_fee'])):('<font color="#FF0000">'.$HTTP_POST_VARS['postal_money_order_fee_error'].'</font>'); 
        $s_message = $s_result ? '':('<font color="#FF0000">'.$HTTP_POST_VARS['postal_money_order_fee_error'].'</font>'); 
      } else {
        $s_message = $s_result ? '':('<font color="#FF0000">'.$HTTP_POST_VARS['postal_money_order_fee_error'].'</font>'); 
      }
    ...
    function process_button() {
      ...
      $s_message = $HTTP_POST_VARS['postal_money_order_fee_error']?$HTTP_POST_VARS['postal_money_order_fee_error']:sprintf(MODULE_PAYMENT_POSTALMONEY_ORDER_TEXT_MAILFOOTER, $currencies->format($total), $currencies->format($HTTP_POST_VARS['postal_money_order_fee']));
      
      return tep_draw_hidden_field('postal_money_order_message', htmlspecialchars($s_message)). tep_draw_hidden_field('postal_money_order_fee', $HTTP_POST_VARS['postal_money_order_fee']);
      //return false;
    }
    ...
    function before_process() {
      ...
      $this->email_footer = str_replace("\r\n", "\n", $HTTP_POST_VARS['postal_money_order_message']);
      return false;
    }
...
0,3000円に入れると、0円から3000円までの金額が決済可能。設定範囲外の決済は不可。', '6', '0', now(), ".$this->site_id.")");
    ...
    function keys() {
      return array('MODULE_PAYMENT_POSTALMONEYORDER_STATUS', 'MODULE_PAYMENT_POSTALMONEYORDER_ZONE', 'MODULE_PAYMENT_POSTALMONEYORDER_ORDER_STATUS_ID', 'MODULE_PAYMENT_POSTALMONEYORDER_SORT_ORDER', 'MODULE_PAYMENT_POSTALMONEYORDER_PAYTO', 'MODULE_PAYMENT_POSTALMONEYORDER_COST', 'MODULE_PAYMENT_POSTALMONEYORDER_MONEY_LIMIT');
    }
  }
?>
