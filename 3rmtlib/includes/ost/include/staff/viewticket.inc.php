...
<table width="100%" cellpadding="2" cellspacing="0" border="0">
    ...
    <tr><td colspan=2 class="msg">件名: <?=Format::htmlchars($ticket->getSubject())?></td></tr>
    <tr>
        ...
        <form name=action action='tickets.php?id=<?=$id?>' method=post class="inline" >
            ...
            <select id="do" name="do" 
              onChange="this.form.ticket_priority.disabled=strcmp(this.options[this.selectedIndex].value,'change_priority','reopen','overdue')?false:true;">
                ...
                <?if(!$ticket->isoverdue()){ ?>
                <option value="overdue" <?=$info['do']=='overdue'?'selected':''?> >期限設定</option>
                <?}?>
                ...
                <?if($thisuser->canCloseTickets()){
                    ...
                    if($ticket->isOpen()){?>
                     <option value="close" <?=$info['do']=='close'?'selected':''?> >Ticket終了</option>
                    <?}else{?>
                        <option value="reopen" <?=$info['do']=='reopen'?'selected':''?> >Ticket再開</option>
                    <?}
                }?>
                <?php
                 if($thisuser->canManageBanList()) {
                    if(!$emailBanned) {?>    
                        <option value="banemail" ><?=$ticket->isOpen()?'受信拒否':'電子メール'?></option>
                    <?}else{?>
                ...
                <option value="delete" >Ticket削除</option>
                <?}?>
            </select>
...
if(($resp=db_query($sql)) && ($notes=db_num_rows($resp))){
    $display=($notes>5)?'none':'block'; //Collapse internal notes(注意書き) if more than 5.
?>
<div align="left">
    <a class="Icon note" href="#" onClick="toggleLayer('ticketnotes'); return false;">注意書き (<?=$notes?>)</a><br><br>
...
<?} ?>
<div align="left">
    <a class="Icon thread" href="#" onClick="toggleLayer('ticketthread'); return false;">回答一覧</a>
    <div id="ticketthread">
  <?
      //get messages
                ...
                <p>
                    <form action="tickets.php?id=<?=$id?>#reply" name="reply" id="replyform" method="post" enctype="multipart/form-data">
                        ...
                        <div style="margin-top: 3px;">
                            ...
                            if($ticket->isOpen()){?>
                            <label><input type="checkbox" name="ticket_status" id="l_ticket_status" value="Close" <?=$checked?> > 返信後にTicketを終了する</label>
                            <?}else{ ?>
                ...
                <p>
                    <form action="tickets.php?id=<?=$id?>#notes" name="notes" class="inline" method="post" enctype="multipart/form-data">
                        ...
                        <div style="margin-top: 3px;">
                            ...
                            if($ticket->isOpen()){?>
                            <label><input type="checkbox" name="ticket_status" id="ticket_status" value="Close" <?=$checked?> > 投稿後にTicketを終了する</label>
                            <?}else{ ?>
