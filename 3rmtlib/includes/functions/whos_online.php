...
*/
  ...
  function tep_update_whos_online() {
    ...
    if (tep_session_is_registered('customer_id')) {
      ...
      $wo_full_name = tep_output_string_protected(tep_get_fullname($customer['customers_firstname'], $customer['customers_lastname']));
    } else {
      $wo_customer_id = '';
      $wo_full_name = 'Guest';
    }
...
// remove entries that have expired
    ...
    $stored_customer_query = tep_db_query("
        ...
        where session_id = '" . tep_db_input($wo_session_id) . "' 
          and site_id = '".SITE_ID."'
    ");
    ...
    if ($stored_customer['count'] > 0) {
      ...
      tep_db_query("
          ...
          set customer_id = '" . $wo_customer_id . "', 
              full_name = '" . tep_db_input($wo_full_name) . "', 
              ip_address = '" . tep_db_input($wo_ip_address) . "', 
              time_last_click = '" . tep_db_input($current_time) . "', 
              last_page_url = '" . tep_db_input($wo_last_page_url) . "' 
          where session_id = '" . tep_db_input($wo_session_id) . "'
            and site_id = '".SITE_ID."'
      ");
    } else {
