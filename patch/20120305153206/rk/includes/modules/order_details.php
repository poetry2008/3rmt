...
// Quantity box or information as an input box or text
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      echo '    <td align="center" class ="main" style=" background:#e2e2e2;padding-left:10px;padding-right:20px;">' . $products[$i]['quantity'] . '個';
...
// Product name, with or without link
    ...
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      if ($cart_pro_status['products_status'] == 0 || $cart_pro_status['products_status'] == 3) {
        ...
        echo '    <td class="main" style=" background:#e2e2e2; width:200px;">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?$products[$i]['model'] . '<br>':'').'<b>' . $products[$i]['name'] . '</b>';
      } else {
        echo '    <td class="main" style=" background:#e2e2e2; width:200px;">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . tep_get_prid($products[$i]['id'])) . '">' . $products[$i]['model'] . '</a><br>':'').'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . tep_get_prid($products[$i]['id'])) . '"><b>' . $products[$i]['name'] . '</b></a>';
      }
    } else {
...
// Product price  
    if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
      ...
      // edit total 
      echo '    <td align="right" class="main" style="background:#e2e2e2"><span id="pri_'.$products[$i]['id'] .'">';
