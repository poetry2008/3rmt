...
?>
      ...
      <p>
        RMTジャックポットでは、<?php echo $po_game_c; ?>の予約サービスを行っております。<br>
...
<?php
    ...
    if (tep_session_is_registered('customer_id')) {
      ...
      $from_name = isset($_POST['yourname'])?$_POST['yourname']:'';
      $from_email_address = isset($_POST['from'])?$_POST['from']:'';
    }
...
<?php
      ...
      if (tep_session_is_registered('customer_id')) {
        ...
        $your_name_prompt = tep_draw_input_field('yourname', (($fromname_error == true) ? $_POST['yourname'] : (isset($_GET['yourname'])?$_GET['yourname']:'')));
        if ($fromname_error == true) $your_name_prompt .= '&nbsp;<span class="errorText">' . TEXT_REQUIRED . '</span>';
        $your_email_address_prompt = tep_draw_input_field('from', (($fromemail_error == true) ? ($_POST['from']) : (isset($_GET['from'])?$_GET['from']:'')) , 'size="30"') . '&nbsp;&nbsp;携帯電話メールアドレス推奨';
...
<?php
      if($error == true) {
        echo '<span class="errorText"><b>入力した内容に誤りがございます。正しく入力してください。</b></span><br><br>';
      }
?>
...
?>
        ...
        <tr>
          ...
          <td class="main">
<?php
if (!isset($timelimit_error)) $timelimit_error=NULL;
