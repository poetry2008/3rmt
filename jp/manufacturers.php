...
<?php 
  $manufacturer_query_raw = "select m.manufacturers_id, m.manufacturers_name, m.manufacturers_image, m.manufacturers_alt, mi.manufacturers_url  from " . TABLE_MANUFACTURERS . " m, " . TABLE_MANUFACTURERS_INFO . " mi where m.manufacturers_id = mi.manufacturers_id and m.manufacturers_id in(select pr.manufacturers_id from ". TABLE_PRODUCTS ." pr, ". TABLE_PRODUCTS_DESCRIPTION ." prd where pr.products_id = prd.products_id)  and languages_id = '" . $languages_id . "' order by manufacturers_name";
...
while ($manufacturer = tep_db_fetch_array($manufacturer_query)){
  ...
  if (tep_db_num_rows($products_query)) {
    ...
    while($products = tep_db_fetch_array($products_query)) {
      ...
      echo '<td align="center" valign="top" class="smallText" width="30%"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '">'.tep_image(DIR_WS_IMAGES.'products/'.$products['products_image'],$products['products_name'],SMALL_IMAGE_WIDTH,SMALL_IMAGE_HEIGHT,'class="image_border"').'<br>' .$products['products_name'] . '</a><br>'.$products_price.'</td>'."\n";
    }
    echo '        </tr>' . "\n";
    echo '      </table>' . "\n";
    echo '    </td>' . "\n";
