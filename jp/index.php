...
// the following cPath references come from application_top.php
 ...
 if (isset($cPath_array)) {
	   ...
	   $header_title.= tep_not_null($seo_category['categories_meta_text']) ? '-' . $seo_category['categories_meta_text'] . '専門のRMTジャックポット' : C_TITLE ;
...
?> 
			<!-- heading title eof//-->
			<p class="comment"><?php echo $seo_category['categories_header_text_' . ABBR_SITENAME]; //seoフレーズ ?></p>
...
?> 
			...
			<br>
			<p class="comment"><?php echo $seo_category['categories_footer_text_' . ABBR_SITENAME]; //seoフレーズ ?></p>
...
<?php } else { // default page ?> 
	  <td valign="top" id="contents">
<?php 
	$contents1 = tep_db_query("select * from ".TABLE_INFORMATION_PAGE." where pID = '10'");  //top1
	$result1 = tep_db_fetch_array($contents1) ;
	$contents2 = tep_db_query("select * from ".TABLE_INFORMATION_PAGE." where pID = '11'");  //top2
