...
function clear_c_image(file_name,input_name){
  ...
  $.ajax({
    ...
    type: 'POST',
    dataType: 'text',
    data: 'image_value='+image_name+'&col_name='+file_name+'&e_cid='+e_cid+'&site_id=<?php echo $site_id;?>', 
    async: false,
    success: function(msg) {
      msg_arr = msg.split('|||'); 
      if (msg_arr[1] == '1') {
        if(msg_arr[0]=='true'){
          $.ajax({
            ...
            type: 'POST',
            dataType: 'text',
            data: 'col_name='+file_name+'&e_cid='+e_cid+'&site_id=<?php echo $site_id;?>', 
            async: false,
            success: function(msg) {
            }
          });
          ...
          confirmg(image_name+' <?php echo TEXT_PRODUCT_IMAGE_DEL_CONFIRM;?>','<?php echo tep_href_link('categories.php?cPath='.$_GET['cPath'].'&cID='.$_GET['cID'].'&c_action='.$_GET['action'].'&mode=ca_delete&site_id='.$site_id) ; ?>&file='+image_name+'&cl='+file_name);
        }
      } else {
        alert('<?php echo NOTICE_NO_ACCESS_READ_TEXT;?>'); 
      }
          ...
          <script language='javascript'>
            ...
            if (isset($_GET['action']) && $_GET['action'] == 'new_product') {
              ...
              ?>
                ...
                <td class="main"><?php echo str_replace(TEXT_PRODUCTS_QUANTITY_TEXT,'',TEXT_MAX) . tep_draw_input_field('inventory_max', $pInfo->max_inventory, 'style="width:78%;" '.($disabled_flag ? 'class="readonly" readonly' : 'id="max_inventory" onblur="inventory_operations(1);"')); ?>&nbsp;<?php echo tep_draw_input_field('inventory_max_contents', isset($pInfo->max_inventory) && $pInfo->max_inventory != '' ? tep_inventory_operations($pInfo->max_inventory,$pInfo->products_id,$site_id) : ($_GET['action'] == 'new_product' ? '' : '0'), 'class="readonly" readonly id="max_inventory_contents" style="text-align:right;width:15%"');?></td>
                </tr>
                <tr>
                <td class="main">&nbsp;</td>
                <td class="main"><?php echo str_replace(TEXT_PRODUCTS_QUANTITY_TEXT,'',TEXT_MIN) . tep_draw_input_field('inventory_min', $pInfo->min_inventory, 'style="width:78%;" '.($disabled_flag ? 'class="readonly" readonly' : 'id="min_inventory" onblur="inventory_operations(0);"')); ?>&nbsp;<?php echo tep_draw_input_field('inventory_min_contents', isset($pInfo->min_inventory) && $pInfo->min_inventory != '' ? tep_inventory_operations($pInfo->min_inventory,$pInfo->products_id,$site_id) : ($_GET['action'] == 'new_product' ? '' : '0'), 'class="readonly" readonly id="min_inventory_contents" style="text-align:right;width:15%"');?></td>
                </tr>
