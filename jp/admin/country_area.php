...
$odd  = 'dataTableRow';
$area_fee_query = tep_db_query("select * from ". TABLE_AREA_FEE ." where fid=$fid");
$i = 0;
while($area_fee_array = tep_db_fetch_array($area_fee_query)){
  ...
  echo '<td>'.$area_fee_array['title'].'</td>';
...
?>
  <a title="有効" onclick="if(confirm('有効にしますか？')){check_on_area('res',<?php echo $area_fee_array['id'];?>,<?php echo $_GET['fid'];?>);}else{return false;}" href="javascript:void(0);"><img border="0" alt="" src="images/icon_status_blue_light.gif"></a>
...
<?php
  echo '<td><a href="javascript:void(0);" onclick="show_text_area('. $area_fee_array['id'] .',this,'. $_GET['fid'] .');">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a></td>';
  echo '</tr>';
  $i++;
}
...
tep_db_close();
?>
<tr><td align="right" colspan="9"><button onclick="show_text_area(0,this,<?php echo $_GET['fid']; ?>);"><?php echo TABLE_BUTTON;?></button></td></tr>
