...
case 'update':
    ...
    case 'save_att_status':
        $has_sql = "select * from ".TABLE_ATTENDANCE_GROUP_SHOW." where is_select='1' and operator_id='".$ocertify->auth_user."'";
        $has_query = tep_db_query($has_sql);
        if($has_row = tep_db_fetch_array($has_query)){
          ...
          while($user_row = tep_db_fetch_array($user_query)){
            $insert_arr['gid'] = '0';
            $insert_arr['user_id'] = $user_row['userid'];
            $insert_arr['is_select'] = '1';
            $insert_arr['operator_id'] = $ocertify->auth_user;
            $insert_arr['att_status'] = $_GET['att_status'];
            $insert_arr['show_type'] = '1';
            $perform_flag =tep_db_perform(TABLE_ATTENDANCE_GROUP_SHOW,$insert_arr);
          }
        }
        tep_redirect(tep_href_link(FILENAME_ROSTER_RECORDS,
    ...
    case 'save_type':
        ...
        if($has_row = tep_db_fetch_array($has_query)){
          ...
          while($user_row = tep_db_fetch_array($user_query)){
            ...
            $insert_arr['is_select'] = '1';
            $insert_arr['operator_id'] = $ocertify->auth_user;
            $insert_arr['att_status'] = '0';
            $insert_arr['show_type'] = $_GET['show_type'];
            $perform_flag =tep_db_perform(TABLE_ATTENDANCE_GROUP_SHOW,$insert_arr);
          }
        }
        tep_redirect(tep_href_link(FILENAME_ROSTER_RECORDS,
...
var attendance_select_date_error = '<?php echo TEXT_ATTENDANCE_SELECT_DATE_ERROR;?>'; 
...
{ 
  ...
  if($j<=$day_num){
    ...
    if($ocertify->npermission>10||$is_manager){
      if($show_group_id!=0){
        echo " onclick='show_replace_attendance_info(this,\"".$date_temp."\",\"".$j."\",\"\",\"\",\"".$show_group_id."\");' >";
      }else{
        echo " onclick='show_replace_attendance_info(this,\"".$date_temp."\",\"".$j."\",\"\",\"\",\"".$show_group_id."\");' >";
      }
    }else{
      if($today <= $date_temp){
        ...
        echo " onclick='show_replace_attendance_info(this,\"".date('Ymd')."\",\"".$j."\",\"\",\"\",\"".$show_group_id."\")'>";
      }
    }
    if($date_temp == $today){
...
//  $temp_user_attenande = tep_all_attenande_by_uid($user_value,$date,$show_group_id,$user_group_arr[$user_value],false);
  ...
  foreach($temp_user_attenande as $user_attenande){
    ...
    if($attendance_info['scheduling_type'] == 0){
      $info_td_attendance_str .=  '<td style="border-width:0px;">';
      if($user_attenande['group_id']!=0){
        $info_td_attendance_str .=  "<span style='display:block; min-height:16px; line-height:16px; float:left' onclick='show_replace_attendance_info(this,\"".$date."\",\"".$k."\",\"".$user_value."\",\"".$user_attenande['attendance_detail_id']."\",\"".$user_attenande['group_id']."\");' style=".$style.">";
      }else{
        $info_td_attendance_str .= "<span style='display:block; min-height:16px; line-height:16px; float:left' onclick='show_replace_attendance_info(this,\"".$date."\",\"".$k."\",\"".$user_value."\",\"".$user_attenande['attendance_detail_id']."\",\"".$user_attenande['group_id']."\");' style='cursor:pointer;'>";
      }
      ...
      $info_td_attendance_str .=  "<td style='border-width:0px;".($attendance_info['scheduling_type'] == 1 && in_array($attendance_info['src_text'],$color_array) ? 'color:#FFFFFF;' : '')."' bgcolor='".$attendance_info['src_text']."'>";
      if($user_attenande['group_id']!=0){
        $info_td_attendance_str .=  "<span style='display:block; min-height:16px; line-height:16px; float:left' onclick='show_replace_attendance_info(this,\"".$date."\",\"".$k."\",\"".$user_value."\",\"".$user_attenande['attendance_detail_id']."\",\"".$user_attenande['group_id']."\");' style=".$style.">";
      }else{
        $info_td_attendance_str .= "<span style='display:block; min-height:16px; line-height:16px; float:left' onclick='show_replace_attendance_info(this,\"".$date."\",\"".$k."\",\"".$user_value."\",\"".$user_attenande['attendance_detail_id']."\",\"".$user_attenande['group_id']."\");' style='cursor:pointer;'>";
    ...
    if($replace_user_row = tep_db_fetch_array($replace_query) && !in_array($user_attenande['attendance_detail_id'],$replace_array)){
      ...
      if($replace_attendance_info['scheduling_type']==0){
        ...
        $info_td_attendance_str .= '<span class="rectangle" style="display:block; min-height:16px; line-height:16px; float:left;background-color:'.$replace_attendance_info['src_text'].';">&nbsp;</span>';
      }
  ...
  while($other_replace_user_row = tep_db_fetch_array($other_replace_query)){
    ...
    if($replace_attendance_info['scheduling_type'] == 0){
      $info_td_attendance_str .=  '<td style="border-width:0px;">';
      $info_td_attendance_str .= "<span style='display:block; min-height:16px; line-height:16px; float:left' onclick='show_replace_attendance_info(this,\"".$date."\",\"".$j."\",\"".$user_value."\",\"".$other_replace_user_row['attendance_detail_id']."\",\"".$show_group_id."\")' >";
      ...
      $info_td_attendance_str .=  "<td style='border-width:0px;".($replace_attendance_info['scheduling_type'] == 1 && in_array($replace_attendance_info['src_text'],$color_array) ? 'color:#FFFFFF;' : '')."' bgcolor='".$replace_attendance_info['src_text']."'>";
      $info_td_attendance_str .= "<span style='display:block; min-height:16px; line-height:16px; float:left' onclick='show_replace_attendance_info(this,\"".$date."\",\"".$j."\",\"".$user_value."\",\"".$other_replace_user_row['attendance_detail_id']."\",\"".$show_group_id."\")' >";
      $info_td_attendance_str .=  $replace_attendance_info['short_language'];
    }
...
while($j<=$day_num)
{
  ...
  if($ocertify->npermission>10||tep_is_group_manager($ocertify->auth_user)){
    ...
    if($today <= $date){
      echo " onclick='show_replace_attendance_info(this,\"".$date."\",\"".$j."\",\"\",\"\",\"".$show_group_id."\")' >";
    }else{
      echo " onclick='show_replace_attendance_info(this,\"".date('Ymd')."\",\"".$j."\",\"\",\"\",\"".$show_group_id."\")'>";
    }
  }
      ...
      foreach($show_select_group_user as $u_list){
        ...
        if($ocertify->auth_user==$u_list||$ocertify->npermission>'10'||in_array($ocertify->auth_user,$manager_list)){
          if($date>=$today||!empty($user_replace)){
            $info_td_attendance_str .=  " onclick='show_replace_attendance_info(this,\"".$date."\",\"".$j."\",\"".$u_list."\",\"".$attendance_row['attendance_detail_id']."\",\"".$show_group_id."\")' ";
          }
        }else{
          $v_att = false;
        }
    ...
    if($last_att_id!=0&&$last_att_id!=$uatt_arr['attendance_detail_id']){
      ...
      */
      ...
      if($ocertify->auth_user==$uatt_arr['user_id']||$ocertify->npermission>'10'){
        if($date>=$today||!empty($user_replace)){
          echo " onclick='show_replace_attendance_info(this,\"".$date."\",\"".$j."\",\"".$uatt_arr['user_id']."\",\"".$attendance_user_row['attendance_detail_id']."\",\"".$show_group_id."\")' ";
        }
      }else{
        $v_att =false;
      }
...
if($show_ulist_flag){
  ...
  //不在排班组的请假
    ...
    foreach($all_replace_att as $row_replace_att){
      ...
      echo "<span>";
      echo "<a href='javascript:void(0)' ";
      echo " onclick='show_replace_attendance_info(this,\"".$date."\",\"".$j."\",\"".$row_replace_att['user']."\",\"".$row_replace_att['attendance_detail_id']."\",\"".$show_group_id."\")' ";
      echo " >";
