...
#000000;background-color: darkred;" class="dataTableContent"
	...
	$orders_info_raw = tep_db_query("select payment_method from ".TABLE_PREORDERS." where orders_id = '".$_GET['oid']."'"); 
	$orders_info = tep_db_fetch_array($orders_info_raw); 

	echo urlencode($orders_info['payment_method']).'_'.tep_check_pre_order_type($_GET['oid']).'_';
	echo tep_get_preorder_canbe_finish($_GET['oid'])?1:0;
	//取得 支付类型 及 支付方法
