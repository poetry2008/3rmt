...
<?php } ?>
  ...
  <tr>
    ...
    <td class="main">
    <?php echo tep_draw_checkbox_field('q_3_1', '1', $oq['q_3_1'], '', 'id="q_3_1" onchange="change_option(this)" onpropertychange="propertychange_option(this)" onclick="q_3_2()"');?><?php echo tep_draw_input_field('q_3_2_m', $oq['q_3_2'] && $oq['q_3_2'] != '0000-00-00' ? date('m', strtotime($oq['q_3_2'])) : ($pay_time?date('m', strtotime($pay_time)):''), 'id="q_3_2_m" size="2" class="questions_date" readonly');?>月<?php echo tep_draw_input_field('q_3_2_d', $oq['q_3_2'] && $oq['q_3_2'] != '0000-00-00' ? date('d', strtotime($oq['q_3_2'])) : ($pay_time?date('d', strtotime($pay_time)):''), 'id="q_3_2_d" size="2" class="questions_date" readonly');?>日 → 金額は<b><?php echo $new_price;?><?php echo tep_draw_hidden_field('q_3_3', $oq['q_3_3']?($oq['q_3_3'] != $new_price ? $new_price : $oq['q_3_3']):$new_price, 'size="10" class="questions_date" readonly style="text-align:right;font-weight:bold;font-size:12px"');?></b>円ですか？→<?php echo tep_draw_checkbox_field('q_3_4', '1', $oq['q_3_4'], '','id="q_3_4" onchange="change_option(this)" onpropertychange="propertychange_option(this)"');?>はい</td>
<?php if (!$oq['q_8_1']) { ?>
...
<div align="right" id="orders_questions_submit_div" style="display:none;">
  ...
  <?php
      for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
        ...
        $r = explode('//',$pinfo['products_attention_1']);
        echo '    <tr class="dataTableRow">' . "\n" . 
       '      <td class="dataTableContent" valign="top" align="right">' . $order->products[$i]['qty']. tep_get_full_count($order->products[$i]['qty'], $r[1], $order->products[$i]['rate']) . '&nbsp;x</td>' . "\n" .
