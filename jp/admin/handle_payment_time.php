...
if ($orders_status_res) {
  $status_info_raw = tep_db_query('select * from '.TABLE_ORDERS_STATUS." where orders_status_id = '".(int)$orders_status_res['orders_status_id']."'");
  $status_info = tep_db_fetch_array($status_info_raw); 
  if ($status_info) {
    if ($status_info['is_pay_time'] == '1') {
