            ...
            foreach($update_totals as $update_totals_key=>$update_totals_value){
              ...
              if($update_totals_value['class'] == 'ot_custom' && trim($update_totals_value['title']) != '' && trim($update_totals_value['value']) != ''){
                ...
                if($totals_email_i != $update_totals_key){
                  ...
                  $totals_email_str .= '▼'.$update_totals_value['title'].$totals_temp_str.'：'.str_replace(TEXT_MONEY_SYMBOL,'',$currencies->format($update_totals_value['value'])); 
                }
              }
            }
            foreach($update_totals as $update_totals_key=>$update_totals_value){
...
while ($order_history = tep_db_fetch_array($order_history_query)) {
  ...
  function add_option(){
    ...
    add_str += '<tr><td class="smallText" align="left"><?php echo EDIT_ORDERS_TOTALDETAIL_READ_ONE;?></td>'
            ...
            +'</td><td class="smallText" align="right"><input id="update_total_'+add_num+'" value="" size="6" onkeyup="clearNoNum(this);price_total(\'<?php echo TEXT_MONEY_SYMBOL;?>\');" name="update_totals['+add_num+'][value]"><input type="hidden" name="update_totals['+add_num+'][class]" value="ot_custom"><input type="hidden" name="update_totals['+add_num+'][total_id]" value="0"><?php echo TEXT_YEN;?></td>'
          ...
          foreach ($TotalsArray as $TotalIndex => $TotalDetails) {
            ...
            if ($TotalDetails["Class"] == "ot_total") {
              ...
              echo '  <tr>' . "\n" .
                ...
                '    <td align="right" class="' . $TotalStyle . '">' . "<input name='update_totals[$TotalIndex][value]' id='update_total_".$TotalIndex."' onkeyup='clearNoNum(this);price_total(\"".TEXT_MONEY_SYMBOL."\");' size='6' value='" . $TotalDetails["Price"] . "'>" .TEXT_YEN. 
                "<input type='hidden' name='update_totals[$TotalIndex][class]' value='" . $TotalDetails["Class"] . "'>" . 
                "<input type='hidden' name='update_totals[$TotalIndex][total_id]' value='" . $TotalDetails["TotalID"] . "'>" . 
                '    <td align="right" class="' . $TotalStyle . '"><b>' . tep_draw_separator('pixel_trans.gif', '1', '17') . '</b>' . 
                '   </td>' . "\n" .
