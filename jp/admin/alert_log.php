...
<?php 
 ...
 if($_GET['sort'] == 'from_notice'){
  ...
  // 获取提醒日志信息
  $memo_query_str = $ocertify->npermission == 31 ? '' : "(bb.manager='".$ocertify->auth_user."' or bb.author='".$ocertify->auth_user."' or bb.allow='all' or bb.allow like '%:".$ocertify->auth_user."' or bb.allow like '%:".$ocertify->auth_user.",%' or bb.allow like '%,".$ocertify->auth_user.",%' or bb.allow like  '%,".$ocertify->auth_user."') and ";
  ...
  $s_select_reply="select n.id id,n.type type,n.title title,n.set_time set_time,n.from_notice from_notice,n.user user,n.created_at created_at,n.is_show is_show,n.deleted deleted from notice n left join ".TABLE_BULLETIN_BOARD_REPLY." br on n.from_notice=br.id join ".TABLE_BULLETIN_BOARD." bb on br.bulletin_id=bb.id where (bb.author='$ocertify->auth_user' or bb.manager='$ocertify->auth_user' or bb.allow='all' or bb.allow like '%:$ocertify->auth_user%' or bb.allow like '%:$ocertify->auth_user,%' or bb.allow like '%,$ocertify->auth_user,%' or bb.allow like '%,$ocertify->auth_user') and n.type=2 and time_format(timediff(now(),n.created_at),'%H')<".$alarm_day*24;
...
if ($rec_c % 2) {
  ...
  $nowColor = $odd;
    ...
    if($arec['type'] == '0'){
      ...
      $user_info = tep_get_user_info($alert_user);
      $alert_user = $user_info['name'];
      $alert_button_name = NOTICE_BULLETIN_TITLE;
      $alert_button_comment = mb_strlen($arec['title'],'utf-8') > 30 ? mb_substr($arec['title'],0,30,'utf-8').'...' : $arec['title'];
      $alert_orders_id = ''; 
    }elseif ($arec['type'] == 2){
