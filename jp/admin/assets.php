    ...
    ?>
      ...
      while($row = tep_db_fetch_array($product_history_query)){
        ?>
          ...
          <td align="right"><?php echo $currencies->format($row['final_price'])=='0'.TEXT_MONEY_SYMBOL?TEXT_UNSET_DATA:$currencies->format($row['final_price']);?></td>
          </tr>
          <?php
      }
    ?>
...
<?php
    ...
    if(count($category_asset_arr)==0&&count($products)==0){
      ...
      ?>
        ...
        foreach($sort_category_arr as $key => $v){
          ...
          echo "</td>";
          echo "<td align='right'>";
          echo $currencies->format($products_price_total/$products_quantity_total)=='0'.TEXT_MONEY_SYMBOL?TEXT_UNSET_DATA:$currencies->format($products_price_total/$products_quantity_total);
          echo "</td>";
          echo "<td align='right'>";
  ...
  <?php
    if(isset($_GET['show_status'])&&$_GET['show_status']=='info'
      ...
      echo "</td>";
      echo "<td class='assets_bottom_info'>";
      echo $currencies->format($products_price_sum)=='0'.TEXT_MONEY_SYMBOL?TEXT_UNSET_DATA:$currencies->format($products_price_sum);
      echo "</td>";
      echo "</tr>";
      echo "</table>";
    }else{
