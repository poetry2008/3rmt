  ...
  ?>
    ...
    for ($i=0; $i<sizeof($order->products); $i++) {
      ...
      if ($is_less_option) {
        ...
        echo "<input type='text' class='update_products_qty' id='update_products_new_qty_$orders_products_id' name='update_products[$orders_products_id][qty]' size='2' value='" .  (isset($_POST['update_products'][$orders_products_id]['qty'])?$_POST['update_products'][$orders_products_id]['qty']:$order->products[$i]['qty']) . "' onkeyup='clearLibNum(this);validate_max_quantity(this,\"".$order->products[$i]['id']."\");recalc_order_price(\"".$oID."\", \"".$orders_products_id."\", \"2\", \"".$op_info_str."\",\"".$orders_products_list."\");price_total(\"".TEXT_MONEY_SYMBOL."\");'>";
          echo '<input type="hidden" value="'.  tep_get_quantity($order->products[$i]['id']).  '" id="max_quantity_'.$order->products[$i]['id'].'">';
          echo '<input type="hidden" value="'.  (isset($_POST['update_products'][$orders_products_id]['qty'])?$_POST['update_products'][$orders_products_id]['qty']:$order->products[$i]['qty']) .'" id="last_quantity_'.$order->products[$i]['id'].'">';
      }
