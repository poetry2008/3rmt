...
var avg_div_flag = 1;
	...
	var js_once_pwd = '<?php echo FORDERS_NOTICE_INPUT_ONCE_PWD;?>';
	var js_once_wrong = "<?php echo FORDERS_NOTICE_ONCE_PWD_WRONG;?>";
	var js_final_date = '<?php echo date('Y-m-d', time())?>';
	var js_final_error_date = '<?php echo ERROR_INPUT_RIGHT_DATE;?>';
//todo:修改通性用
<?php
...
?>
	var js_final_languages_id = '<?php echo $languages_id;?>';
	var js_final_id_str = "<?php echo $products_id_str;?>";
	var js_final_info_str = "<?php echo $op_info_str;?>";
</script>
<script language="javascript" src="includes/javascript/admin_final_preorders.js?v=<?php echo $back_rand_info?>"></script>
<style type="text/css">
  ...
  for ($i=0; $i<sizeof($order->products); $i++) {
    ...
    if ($order->products[$i]['attributes'] && sizeof($order->products[$i]['attributes']) > 0) {
      ...
      foreach($all_show_option_id as $t_item_id){
        ...
        $item_default_value = strtr($all_show_option[$t_item_id]['option_info']['value'], array("'"=>"&#39;",'"'=>"&#34;")) == '' ? TEXT_UNSET_DATA : strtr($all_show_option[$t_item_id]['option_info']['value'], array("'"=>"&#39;",'"'=>"&#34;"));
      ...
      foreach ($order->products[$i]['attributes'] as $ex_key => $ex_value) {
        if (!in_array($ex_value['id'], $op_include_array)) {
          ...
          echo "<div class=\"order_option_value\">".$ex_value['option_info']['value']."<input type='hidden' name='update_products[$orders_products_id][attributes][".$ex_value['id']."][value]' value='".strtr($ex_value['option_info']['value'], array("'" => "&#39;",'"'=>"&#34;"))."'></div>";
          echo '</div>'; 
          echo '<div class="order_option_price">';
