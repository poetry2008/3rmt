...
*/
  ...
  //print_r($_SESSION['create_preorder']);
  //unset($_SESSION['preorder_products'][$_GET['oID']]);
  //print_r($_SESSION['preorder_products'][$_GET['oID']]);
...
<?php
  for ($i=0; $i<sizeof($order->products); $i++) {
    ...
    echo '<td class="'.$RowStyle.'" align="right">'.$orders_products_type.'<input type="text" style="text-align:right;" class="once_pwd" name="update_products['.$orders_products_id.'][p_price]" size="9" value="'.(isset($_SESSION['preorder_products'][$_GET['oID']]['price']) ? $_SESSION['preorder_products'][$_GET['oID']]['price'] : tep_display_currency(number_format(abs($order->products[$i]['price']), 2))).'" onkeyup="clearLibNum(this);recalc_preorder_price(\''.$oID.'\', \''.$orders_products_id.'\', \'2\', \''.$op_info_str.'\');" >'.TEXT_MONEY_SYMBOL.'</td>'; 
...
// END OF MAKING ALL INPUT FIELDS THE SAME LENGTH
  ...
  if(isset($_SESSION['orders_update_products'][$_GET['oID']]['customers_add_num'])){
    ...
    if($ot_custom_flag == false){
      ...
      array_pop($TotalsArray);
      if($_SESSION['orders_update_products'][$_GET['oID']]['customers_add_num'] >= 5){
  ...
  foreach ($TotalsArray as $TotalIndex => $TotalDetails) {
    ...
    if ($TotalDetails["Class"] == "ot_total") {
      ...
      echo '  <tr>' . "\n" .
           ...
           '    <td align="right" class="' . $TotalStyle . '">' . "<input name='update_totals[$TotalIndex][value]' id='update_totals_$TotalIndex' onkeyup='clearNewLibNum(this);price_total();recalc_preorder_price(\"".$oID."\", \"".$orders_products_id."\", \"1\", \"".$op_info_str."\");' size='6' value='" . (isset($_SESSION['preorder_products'][$_GET['oID']]['customer'][$TotalIndex]) ? $_SESSION['preorder_products'][$_GET['oID']]['customer'][$TotalIndex] : $TotalDetails["Price"]) . "'>" . 
                "<input type='hidden' name='update_totals[$TotalIndex][class]' value='" . $TotalDetails["Class"] . "'>" . 
                "<input type='hidden' name='update_totals[$TotalIndex][total_id]' value='" . $TotalDetails["TotalID"] . "'>" . 
           '    <td align="right" class="' . $TotalStyle . '"><b>' . tep_draw_separator('pixel_trans.gif', '1', '17') . '</b>' . 
           '   </td>' . "\n" .
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="dataTableRow">
  ...
  <tr>
    <td valign="top" width="40%">
      <table border="0" cellspacing="0" cellpadding="2">
        ...
        <tr>
          ...
          <td class="main"><?php echo tep_draw_checkbox_field('notify_comments', '', isset($_SESSION['orders_update_products'][$_GET['oID']]['notify_comments']) && $_SESSION['orders_update_products'][$_GET['oID']]['notify_comments'] == 0 ? false : true); ?>&nbsp;&nbsp;<b style="color:#FF0000;"><?php echo EDIT_ORDERS_RECORD_READ;?></b></td>
        </tr>
        <tr>
    ...
    ?>
    <?php echo '<b>'.ENTRY_EMAIL_TITLE.'</b>'.tep_draw_input_field('etitle', isset($_SESSION['orders_update_products'][$_GET['oID']]['etitle']) ? $_SESSION['orders_update_products'][$_GET['oID']]['etitle'] : $mail_sql['orders_status_title'],' style="width:230px;" id="mail_title"');?> 
    <br> 
    <br> 
