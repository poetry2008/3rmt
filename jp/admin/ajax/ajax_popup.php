  ...
  if ($c_key < (count($cid_array) - 1)) {
   ...
   if($_GET['cID'] != -1){
    if($disabled){
     ...
     $customers_del =  ' <a class = "new_product_reset" href="javascript:void(0);">'.tep_html_element_button(IMAGE_DELETE, 'onclick="'.(tep_get_orders_by_customers_id($nInfo->customers_id,$nInfo->site_id) > 0 || tep_get_orders_by_customers_id($nInfo->customers_id,$nInfo->site_id) > 0 ? 'if(confirm(\''.TEXT_CUSTOMERS_DELETE_CONFIRM_INFO.'\n'.tep_customers_name($nInfo->customers_id).'\')){if(confirm(\''.TEXT_DEL_NEWS.'\'))' : 'if (confirm(\''.TEXT_DEL_NEWS.'\')){').' toggle_customers_action(\''.tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params($tmp_ex_array) . 'cID=' .  $cInfo->customers_id .  '&action=deleteconfirm').'\', \''.$ocertify->npermission.'\');}"').'</a>';
     if ($cInfo->is_active == '1') {
       if ($ocertify->npermission >= 15) {
