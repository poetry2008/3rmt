...
//图片
  ...
  $src_text = $att_info_res['src_text'];
   ...
   $style_image = 'style="display:none;"';
    ...
    $select_type_image .= tep_html_element_button(ATTENDANCE_IMAGE_SELECT,'onclick="document.attendances.upload_file_image.click()" id="upload_button" '. $show_style .'').'</div>'; 
...
if($row_array['set_time']==0){
  ...
  */
  ...
  for($j=0; $j<count($a_info);$j++){
    ...
    $has_user_select .= '</select><input type="hidden" name="data_as['.$a_info[$j]['u_group'].'][]" value="'.$a_info[$j]['id'].'"><td nowrap="nowrap"><input type="button" value="'.TEXT_DEL_ADL.'" onclick="del_one_person(this)"><input '.$style_hidden_del.'  type="button" onclick="add_person_row(this,\''.$a_info[$j]['u_group'].'\')" value="'.TEXT_ADD_ADL.'"></td>';
    $as_info_row[]['text'] = array(
...
//没有个人排班的时候显示新数据
 ...
 if($user_wage_list != ''){
   ...
   $user_wage_query = tep_db_query("select wage_id,wage_value,start_date,end_date,contents,create_date,update_date,create_user,update_user from ".TABLE_USER_WAGE_INFO." where id in (".$user_wage_list.")"); 
   while($user_wage_array = tep_db_fetch_array($user_wage_query)){
   $show_tep_info[]=$user_wage_array;
