...
*/
  ...
  switch ($_GET['action']) {
/* -----------------------------------------------------
   case 'insert' 新建预约订单状态   
   case 'save' 更新预约订单状态   
   case 'deleteconfirm' 删除预约订单状态   
   case 'delete' 判断该预约状态是否被删除  
------------------------------------------------------*/
    case 'insert':
    case 'save':
      ...
      for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
        ...
        if ($_GET['action'] == 'insert') {
          ...
          //同步对应的邮件模板
          tep_db_query("insert into ". TABLE_MAIL_TEMPLATES ." values(NULL,'PREORDERS_STATUS_MAIL_TEMPLATES_".$orders_status_id."','0','".tep_db_prepare_input($orders_status_name_array[$language_id]).TEXT_PREORDERS_STATUS_MAIL_TITLE."','".TEXT_PREORDERS_STATUS_MAIL_USE_DESCRIPTION."','','','".TEXT_PREORDERS_STATUS_MAIL_DESCRIPTION."','1','1','".$_POST['user_added']."',now(),'".$_POST['user_added']."',now())");
      ...
      if ($_POST['default'] == 'on') {
        tep_db_query("update " . TABLE_CONFIGURATION . " set configuration_value = '" . tep_db_input($orders_status_id) . "' where configuration_key = 'DEFAULT_PREORDERS_STATUS_ID'");
      }

      tep_redirect(tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $orders_status_id));
      break;
    case 'deleteconfirm':
      $oID = tep_db_prepare_input($_GET['oID']);

      $orders_status_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'DEFAULT_PREORDERS_STATUS_ID'");
      $orders_status = tep_db_fetch_array($orders_status_query);
      if ($orders_status['configuration_value'] == $oID) {
        tep_db_query("update " . TABLE_CONFIGURATION . " set configuration_value = '' where configuration_key = 'DEFAULT_PREORDERS_STATUS_ID'");
      }

      tep_db_query("delete from " . TABLE_PREORDERS_STATUS . " where orders_status_id = '" . tep_db_input($oID) . "'");
      //删除对应的邮件模板
      tep_db_query("delete from ". TABLE_MAIL_TEMPLATES ." where flag='PREORDERS_STATUS_MAIL_TEMPLATES_".tep_db_input($oID)."'");
      
      tep_redirect(tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page']));
      break;
    case 'delete':
      $oID = tep_db_prepare_input($_GET['oID']);

      $status_query = tep_db_query("select count(*) as count from " . TABLE_PREORDERS . " where orders_status = '" . tep_db_input($oID) . "'");
      $status = tep_db_fetch_array($status_query);

      $remove_status = true;
      if ($oID == DEFAULT_PREORDERS_STATUS_ID) {
        //该状态是否是默认状态 
        $remove_status = false;
        ...
        $history_query = tep_db_query("select count(*) as count from " . TABLE_PREORDERS_STATUS_HISTORY . " where orders_status_id = '" . tep_db_input($oID) . "'");
...
<script type="text/javascript">
	var js_preorders_status_npermission = '<?php echo $ocertify->npermission;?>';
	var js_preorders_status_self = '<?php echo $_SERVER['PHP_SELF']?>';
	var js_onetime_pwd = '<?php echo JS_TEXT_INPUT_ONETIME_PWD;?>';
	var js_onetime_error = '<?php echo JS_TEXT_ONETIME_PWD_ERROR;?>';
</script>
<script language="javascript" src="includes/javascript/admin_preorders_status.js?v=<?php echo $back_rand_info?>"></script>
<?php 
...
<?php
  ...
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
    ...
    if ( isset($oInfo) && (is_object($oInfo)) && ($orders_status['orders_status_id'] == $oInfo->orders_status_id) ) {
      echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->orders_status_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '                  <tr class="'.$nowColor.'" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\''.$nowColor.'\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $orders_status['orders_status_id']) . '\'">' . "\n";
    }
...
?>
                <td class="dataTableContent" align="right"><?php if ( isset($oInfo) && (is_object($oInfo)) && ($orders_status['orders_status_id'] == $oInfo->orders_status_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $orders_status['orders_status_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
...
<?php
  $heading = array();
  $contents = array();
  $explanation = PREORDERS_STATUS_EXPLANATION_TEXT;
  switch (isset($_GET['action'])?$_GET['action']:null) {
/* -----------------------------------------------------
   case 'new' 右侧新建预约订单状态页面 
   case 'edit' 右侧编辑预约订单状态页面 
   case 'delete' 右侧删除预约订单状态页面
   default 右侧默认页面
------------------------------------------------------*/
    case 'new':
      ...
      $contents = array('form' => tep_draw_form('preorders_status', FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&action=insert', 'post', 'enctype="multipart/form-data"'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="javascript:void(0);">' .  tep_html_element_button(IMAGE_SAVE, 'onclick="check_status_form();"') . '</a> <a class="new_product_reset" href="' . tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page']) .  '">' . tep_html_element_button(IMAGE_CANCEL) . '</a>');
      break;
    case 'edit':
      $heading[] = array('text' => TEXT_INFO_HEADING_EDIT_ORDERS_STATUS);

      $contents = array('form' => tep_draw_form('preorders_status', FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->orders_status_id  . '&action=save', 'post', 'enctype="multipart/form-data"'));
      ...
      $languages = tep_get_languages();
      for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
        $orders_status_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' .  tep_draw_input_field('orders_status_name[' . $languages[$i]['id'] . ']', tep_get_preorders_status_name($oInfo->orders_status_id, $languages[$i]['id']));
      }
    ...
    $contents[] = array('text' => '<input type="hidden" name="site_id" value="'.($os_result?$os_result['site_id']:$site_id).'">');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="javascript:void(0);">' .  tep_html_element_button(IMAGE_SAVE, 'onclick="check_status_form();"') . '</a> <a class="new_product_reset" href="' . tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] .  '&oID=' . $oInfo->orders_status_id) . '">' . tep_html_element_button(IMAGE_CANCEL) . '</a>');
      break;
    case 'delete':
      $heading[] = array('text' => TEXT_INFO_HEADING_DELETE_ORDERS_STATUS);

      $contents = array('form' => tep_draw_form('preorders_status', FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->orders_status_id  . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('text' => '<br>' . $oInfo->orders_status_name);
      if ($remove_status) $contents[] = array('align' => 'center', 'text' => '<br><a href="javascript:void(0);">' . tep_html_element_button(IMAGE_DELETE, 'onclick="check_status_form();"') .  '</a> <a class="new_product_reset" href="' . tep_href_link(FILENAME_PREORDERS_STATUS, 'page=' . $_GET['page'] .  '&oID=' . $oInfo->orders_status_id) . '">' . tep_html_element_button(IMAGE_CANCEL) . '</a>');
      break;
    default:
