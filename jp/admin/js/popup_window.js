...
function popup_window(ele,type,title,list){ 
  ...
  default_value = default_value.replace(/<\/u>/i,"");
  default_value = default_value == text_unset_data ? '' : default_value;
  ...
  var font_size = $(ele).css("font-size");
...
function popup_window_head(title){
  ...
  html_str += '<tr>';
  html_str += '<td width="22"><img width="16" height="16" border="0" title="'+image_icon_info+'" alt="'+image_icon_info+'" src="images/icon_info.gif">&nbsp;</td>';
...
function popup_window_body(default_value,type_html,id){ 
  ...
  html_str += '<tr>';
  html_str += '<td width="55"valign="top">'+text_popup_window_show+'</td>';
  html_str += '<td align="left" colspan="2">'+default_value+'</td>';
  html_str += '</tr>';
  html_str += '<tr>';
  html_str += '<td width="55" valign="top">'+text_popup_window_edit+'</td>';
  html_str += '<td align="left" colspan="2">'+type_html+'</td>';
  html_str += '</tr>';
  html_str += '<tr>';
  html_str += '<td align="center" colspan="3"><input type="button" value="'+image_save+'" onclick="popup_window_save(\''+id+'\');"></td>';
  html_str += '</tr>';  
  html_str += '</table>';
  return html_str;
}
...
function popup_window_save(id){
  var default_value = $("#default_value_"+id).val() == '' ? text_unset_data : $("#default_value_"+id).val();
