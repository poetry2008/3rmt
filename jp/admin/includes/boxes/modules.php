...
<?php 
  if (!check_whether_is_limited(FILENAME_MODULES)) {
    ...
    if(str_replace('/admin/','',$_SERVER['PHP_SELF']) == FILENAME_MODULE_TOTAL){
      echo '<div class="sidebarselected" onclick="window.location.href=\''.tep_href_link(FILENAME_MODULE_TOTAL, '', 'NONSSL').'\';"><span>' . tep_image(DIR_WS_MENU_ICON .  'icon_calculation.gif').'</span><span>' . BOX_MODULES_ORDER_TOTAL . '</span></div>';
    }else{
