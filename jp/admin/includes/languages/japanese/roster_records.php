<?php
define('ATTENDANCE_HEAD_TITLE', 'テンプレート設定');
define('ATTENDANCE_ABBREVIATION', '略語');
define('ATTENDANCE_SCHEDULING_TYPE', '色・アイコン設定');
define('ATTENDANCE_SCHEDULING_TYPE_IMAGE', 'アイコン画像');
define('ATTENDANCE_SCHEDULING_TYPE_COLOR', 'カラーゾーン');

define('ATTENDANCE_PARAM_TEXT', 'パラメータ');
define('ATTENDANCE_SET_TIME', '出勤時間設定');
define('ATTENDANCE_SET_TIME_FIELD', '時間帯');
define('ATTENDANCE_SET_FIELD_TIME', '一日の勤務時間数');
define('ATTENDANCE_WORK_START', '開始時刻');
define('ATTENDANCE_WORK_END', '終了時刻');
define('ATTENDANCE_REST_START', '休憩開始');
define('ATTENDANCE_REST_END', '休憩終了');
define('ATTENDANCE_WORK_HOURS', '出勤時間数');
define('ATTENDANCE_REST_HOURS', '休憩時間数');
define('ATTENDANCE_SORT', '整列順');

define('TEP_ERROR_NULL','入力してください。');
define('IMAGE_NEW_ATTENDANCE', 'テンプレート追加');

//roster_records页面
define('ROSTER_TITLE_TEXT', 'シフト・出勤');
define('TEXT_GROUP_SELECT', 'グループ');
define('TEXT_GROUP_USER_LIST', 'スタッフ指定');
define('TEXT_ALL_GROUP', '全て');
define('TEXT_ADL_SELECT', '出勤内容');
define('COMPANY_SYSTEM_SELECT', '出勤グループ');
define('TEXT_ADD_ADL', '追加');
define('TEXT_DEL_ADL', '削除');
define('TEXT_TYPE_SELECT', '予定を繰り返す');
define('TEXT_CALENDAR_REPEAT_TYPE_NO','繰り返しなし');
define('TEXT_CALENDAR_REPEAT_TYPE_EVERY_WEEK','毎週');
define('TEXT_CALENDAR_REPEAT_TYPE_WEEK','隔週');
define('TEXT_CALENDAR_REPEAT_TYPE_WEEK_HEAD','隔');
define('TEXT_CALENDAR_REPEAT_TYPE_WEEK_TAIL','週');
define('TEXT_CALENDAR_REPEAT_TYPE_MONTH','毎月');
define('TEXT_CALENDAR_REPEAT_TYPE_MONTH_WEEK','毎月の固定曜日');
define('TEXT_CALENDAR_REPEAT_TYPE_YEAR','毎年');
define('IMAGE_HISTORY','出勤履歴');
define('IMAGE_REPLACE_ATTENDANCE','シフト変更');
define('TEXT_ADL_SELECT_USER','出勤内容');
define('TEXT_ADL_SELECT_USER_TEXT','出勤時間帯');
define('TEXT_REPLACE_ADL','出勤内容変更');
define('TEXT_ALLOW_USER','責任者');
define('TEXT_ALLOW_STATUS','責任者確認');
define('TEXT_REPLACE_NOT_ALLOW','--');
define('TEXT_REPLACE_IS_ALLOW','許可');
define('TEXT_ALLOW_START','終始時刻');
define('TEXT_ALLOW_END','終了日時');
define('TEXT_ADL_TEXT_INFO','コメント');
define('TEXT_LEAVE_ONE_DAY','--');
define('TEXT_SELECT_USER','出勤者');
define('TEXT_DELETE_REMIND','削除しますか？');
define('TEXT_ONLY_USER_ATTENDANCE','スタッフ設定');
define('TEXT_WARN_CHANGE_ATTENDANCE_OVERLAP','「開始時刻」と「終了時刻」の範囲内に設定してください。');

define('SHOW_ALL_ATT_STATUS','実績表示');
define('SHOW_ERROR_ATT_STATUS','遅刻・早退のみ表示');
define('SHOW_NULL_ATT_STATUS','表示なし');
define('TEXT_ATT_SET_VALUE','指定在席時間数小計');
define('TEXT_ATT_ACTUAL_VALUE','実際在席時間数小計');
define('TEXT_REMIND_CHOICE_SELECT','*必須');
define('TEXT_ATTENDANCE_SETTING', '各種設定');
define('TEXT_ATTENDANCE_SETTING_USER', 'スタッフの追加・修正');
define('TEXT_ATTENDANCE_SETTING_GROUP', '出勤グループの追加・修正');
define('TEXT_ATTENDANCE_SETTING_GROUP_CONFIG', 'グループ設定');
define('TEXT_ATTENDANCE_SETTING_CHANGE', '出勤予定変更');
define('TEXT_ATTENDANCE_SETTING_MOVE', 'シフトの追加・削除');
define('TEXT_ATTENDANCE_SETTING_TITLE', 'シフト');
define('TEXT_ATTENDANCE_SETTING_SHOW', '表示設定');
define('TEXT_ATTENDANCE_TABLE_MODE', 'スタッフ名ごとに表示');
define('TEXT_ATTENDANCE_CALENDAR_MODE', '出勤時間ごとに表示');
define('TEXT_ATTENDANCE_DATE', '日付');
define('TEXT_PARAM_TYPE','勤務形態');
define('TEXT_PRRAM_ATTENDANCE_NUM','勤務時間数');
define('TEXT_ATTENDANCE_SELECT_TITLE', 'スタッフ');
define('TEXT_ATTENDANCE_SETTING_PAYROLLS', '給与関連設定');
define('TEXT_ATTENDANCE_CHANGE_USER', '・出勤変更スタッフ');
define('TEXT_ATTENDANCE_CHANGE_DETAIL_INFO', '・入力項目');
define('TEXT_ATTENDANCE_PLAN', '出勤予定');
define('TEXT_ATTENDANCE_DATE_ERROR', '正しく入力してください');
define('TEXT_ATTENDANCE_DATE_END', '終了');
define('TEXT_ATTENDANCE_DATE_END_CONFIRM', '本当に終了しますか？');
?>

