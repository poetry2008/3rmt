<?php
define('TEXT_ORDERS_SETTING','设置订单管理的显示方法');
define('TEXT_PREORDERS_SETTING','设置预约管理的显示方法');
define('TEXT_ORDERS_SELECT_SITE','网站');
define('TEXT_ORDERS_SELECT_PRICE','订单总额');
define('TEXT_ORDERS_SELECT_TIME','送达时间');
define('TEXT_ORDERS_SELECT_DATE','订购日期');
define('TEXT_ORDERS_SELECT_STATUS','状态');
define('TEXT_SELECT_ASC','升序');
define('TEXT_SELECT_DESC','降序');
define('TEXT_SAVE','保存');
define('TEXT_PREORDERS_SELECT_CUSTOMER','顾客名');
define('TEXT_PREORDERS_SELECT_DATE','预约日期');
define('TEXT_PERSONAL_SETTING_SITE_ERROR','请至少选择一个。');
define('TEXT_PERSONAL_SETTING_SORT_ERROR','请正确选择。');
define('TEXT_PERSONAL_SETTING_LANGUAGE_OPTION','默认语言选择');
define('TEXT_ONE_TIME_CONFIG_SAVE','保存成功');
define('TEXT_PERSONAL_SETTING_SHOW_TRANSACTION_FINISH', '显示交易完成的订单');
define('TEXT_PERSONAL_SETTING_HIDE_TRANSACTION_FINISH', '隐藏交易完成的订单');
define('TEXT_PERSONAL_SETTING_PREORDERS_SHOW_TRANSACTION_FINISH', '显示预约终止的订单');
define('TEXT_PERSONAL_SETTING_PREORDERS_HIDE_TRANSACTION_FINISH', '隐藏预约终止的订单');
define('TEXT_PERSONAL_SETTING_SOUND', '声音');
define('TEXT_PERSONAL_SETTING_NOTIFICATION_SOUND', '响铃通知');
define('TEXT_PERSONAL_SETTING_ON_TEXT', '开');
define('TEXT_PERSONAL_SETTING_OFF_TEXT', '关');
