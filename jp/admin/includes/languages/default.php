...
define('TEXT_PREORDERS_STATUS_MAIL_DESCRIPTION','名前：${USER_NAME}<br>メールアドレス：${USER_MAIL}<br>予約日：${PREORDER_DATE}<br>予約番号：${PREORDER_NUMBER}<br>お支払方法：${PAYMENT}<br>お支払金額：${ORDER_TOTAL}<br>注文ステータス：${ORDER_STATUS}<br>メールに記載する注意書き：${ORDER_COMMENT}<br>サイト名：${SITE_NAME}<br>サイトのURL：${SITE_URL}<br>お問い合わせ用メールアドレス：${SUPPORT_MAIL}<br>確保期限：${ENSURE_TIME}<br>希望個数：${PRODUCTS_QUANTITY}<br>商品名：${PRODUCTS_NAME}<br>商品単価：${PRODUCTS_PRICE}<br>小計：${SUB_TOTAL}<br>会社名称：${COMPANY_NAME}<br>会社の住所：${COMPANY_ADDRESS}<br>会社の電話番号：${COMPANY_TEL}<br>担当者メール：${STAFF_MAIL}<br>担当者名：${STAFF_NAME}<br>Eメール署名：${SIGNATURE}<br>ユーザ情報：${USER_INFO}<br>年：${YEAR}<br>月：${MONTH}<br>日：${DAY}<br>HTTPS：${HTTPS_SERVER}');
