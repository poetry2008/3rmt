...
function show_address_book(_this){
  //显示 地址本
  if(_this.value == 'show_address'){
  address_list = window.document.getElementById('address_book_list');
  address_list.style.display = 'block';
  }else if(_this.value == 'create_address'){
    create_address_book();
  }else{
    //所有信息隐藏
  }
...
function show_shipping_method(){
  shipping_list_method = window.document.getElementById('shipping_list');
  shipping_list_method.style.display = 'block';
}
...
function set_torihiki_date(shipping_code,work_time,start_time){
  ...
  torihiki_info_list_t = window.document.getElementById('torihiki_info_list');
...
function get_torihiki_time_list(work_time,start_time,torihiki_date,radio_name,check_time){
  ...
  show_row = 0;
  torihiki_time_str = "<select name='"+radio_name+"'>";
  torihiki_time_str += "<option value=''>selected_time</oprion>";
  row_num = 0;
  for(s_hour;s_hour<=work_end_hour;s_hour++){
    ...
    for(s_mim;s_mim<end_mim;){
      ...
      s_mim+=14;
      e_start = s_mim;
      torihiki_time_str += "<option ";
      torihiki_time_str += " value='";
      ...
      if(check_time == s_hour+":"+format_time(s_start)+"-"+s_hour+":"+format_time(e_start)){
        torihiki_time_str += " selected='true' ";
      }
      torihiki_time_str += " >";        
