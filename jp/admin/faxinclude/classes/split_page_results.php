...
// $num_pages now contains int of pages needed unless there is a remainder from division
      ...
      if ($num_pages > 1) {
        ...
        $form_action = tep_admin_href_link(TAX_ORDER_FILE, '', 'ADMINSSL');
        $display_links = '<form name="pages" action="' . $form_action .'" method="get">';

        if ($current_page_number > 1) {
          $display_links .= '<a href="' . tep_admin_href_link(basename($PHP_SELF),
            $parameters . $page_name . '=' . ($current_page_number - 1), 'ADMINSSL') . '" class="splitPageLink">' . PREVNEXT_BUTTON_PREV . '</a>&nbsp;&nbsp;';
        } else {
        ...
        if (($current_page_number < $num_pages) && ($num_pages != 1)) {
          $display_links .= '&nbsp;&nbsp;<a href="' .
            tep_admin_href_link(basename($PHP_SELF), $parameters . $page_name . '='
                . ($current_page_number + 1), 'ADMINSSL') . '" class="splitPageLink">' . PREVNEXT_BUTTON_NEXT . '</a>';
        } else {
