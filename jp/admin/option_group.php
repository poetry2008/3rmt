    ...
    if (isset($_GET['search'])) {
      if ($_GET['search'] == '2') {
        $group_query_raw = 'select * from '.TABLE_OPTION_GROUP.' where name =
          \''.tep_replace_full_character($_GET['keyword']).'\' order by sort_num asc';
      } else {
        $group_query_raw = 'select * from '.TABLE_OPTION_GROUP.' where name like \'%'.tep_replace_full_character($_GET['keyword']).'%\' order by sort_num asc';
      }
    } else {
      $group_query_raw = 'select * from '.TABLE_OPTION_GROUP.' order by sort_num asc';
    }
