...
/*------------------------------
 ...
 返回值：支付方法的名称(string)
 -----------------------------*/    
  function specialOutput($payment, $show_type = false, $p_info = null) {
    $p = $this->getModule($payment);
    if(method_exists($p,'specialOutput')){
      return $p->specialOutput($this->session_paymentvalue_name, $show_type, $p_info);
    }
    return ''; 
  }
/*-----------------------------
 ...
 ----------------------------*/
  public static   function makePaymentListPullDownMenu(&$payment_method = "",$site_id='',$c_chk='',$type='order') {
    ...
    for($i=0; $i<sizeof($payment_array[0]); $i++) {
      ...
      if(isset(self::$use_payment)&&self::$use_payment!=''&&self::$use_payment!=null){
      if($payment_method==$payment_array[0][$i]){
        $continue_flag = false;
      }
      }
      if($continue_flag){
