...
0,3000円に入れると、0円から3000円までの金額が決済可能。設定範囲外の決済は不可。', '6', '0', now(), ".$this->site_id.")");
    ...
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added, site_id) values ('ポイント', 'MODULE_PAYMENT_BUYING_IS_GET_POINT', '1', 'ポイント', '6', '1', 'tep_cfg_payment_new_checkbox(', now(), ".$this->site_id.");");
...
/*----------------------------
 ...
 ---------------------------*/
  function admin_get_payment_info_comment($customers_email,$site_id,$orders_type,$gray_single){
    ...
    if ($exists_single) {
      $orders_status_history_temp_query = tep_db_query("select payment_method,orders_id from ". $orders_type_str ." where customers_email_address='". $customers_email ."' and site_id='".$site_id."' and payment_method='".TS_MODULE_PAYMENT_BUYING_TEXT_TITLE."' and is_gray != '1' and date_purchased >= '".$customers_info['customers_info_date_account_created']."' limit 0,1");
    } else {
      $orders_status_history_temp_query = tep_db_query("select payment_method,orders_id from ". $orders_type_str ." where customers_email_address='". $customers_email ."' and site_id='".$site_id."' and payment_method='".TS_MODULE_PAYMENT_BUYING_TEXT_TITLE."' and is_gray != '1' limit 0,1");
    }
    ...
    if($orders_num_rows > 0){
      if ($exists_single) {
        $orders_status_history_query = tep_db_query("select payment_method,orders_id from ". $orders_type_str ." where customers_email_address='". $customers_email ."' and site_id='".$site_id."' and payment_method='".TS_MODULE_PAYMENT_BUYING_TEXT_TITLE."' and is_gray != '1' and date_purchased >= '".$customers_info['customers_info_date_account_created']."' order by orders_id desc limit 0,1");
      } else {
        $orders_status_history_query = tep_db_query("select payment_method,orders_id from ". $orders_type_str ." where customers_email_address='". $customers_email ."' and site_id='".$site_id."' and payment_method='".TS_MODULE_PAYMENT_BUYING_TEXT_TITLE."' and is_gray != '1' order by orders_id desc limit 0,1");
      }
