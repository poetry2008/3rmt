...
if (!isset($_POST['from'])) $_POST['from'] = NULL; //del notice
      ...
      $from_name = stripslashes(tep_get_fullname($_POST['firstname'], $_POST['lastname'])); 
    ...
    if (isset($_GET['action']) && ($_GET['action'] == 'process') && ($error == false)) { 
      ...
      if (tep_session_is_registered('customer_id')) {
          ...
          $pre_replace_info_arr = array($_POST['products_name'], $_POST['quantity'].NUM_UNIT_TEXT.' '.tep_get_full_count2($_POST['quantity'],$_POST['products_id']), $payment_name_str, tep_get_fullname($account_values['customers_firstname'],$account_values['customers_lastname']), STORE_NAME, HTTP_SERVER, $preorder_id, stripslashes($_POST['yourmessage']), $mail_option_str);
        ...
        if (tep_db_num_rows($exists_customer_raw)) {
          ...
          if ($exists_customer_res['is_active'] == 0) {
            ...
            $pre_replace_info_arr = array($_POST['products_name'], $_POST['quantity'].NUM_UNIT_TEXT.' '.tep_get_full_count2($_POST['quantity'],$_POST['products_id']), $payment_name_str, $from_name, STORE_NAME, HTTP_SERVER, $preorder_id, stripslashes($_POST['yourmessage']), $mail_option_str);
