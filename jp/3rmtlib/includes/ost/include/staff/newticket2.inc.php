...
if(count($_POST)){//如果提交后返回
$info=($_POST && $errors)?Format::input($_POST):array(); //on error...use the post data
}else {
  $info['email'] = urldecode($_GET['email']);
  $info['source'] = urldecode($_GET['source']);
  $info['name'] = urldecode($_GET['name']);
  $info['topicId'] = urldecode($_GET['topicid']);
  $department = new Topic($info['topicId']);
  $info['deptId'] = $department->getDeptId();
  unset($department);
}
?>
...
?>
    ...
    <tr>
        ...
        <td>
            ...
            <input class="button" type="button" name="cancel" value="キャンセル" onClick='closeme();'>    
        </td>
    </tr>
  </form>
</table>
