...
------------------------------------ */
  public function prepareFormWithParent($item_id){
    ...
    if ($this->has_text_comment) {
      ...
      $formString .= "<tr><td width='25%' align='left'>".TEXT_ITEM_TEXT_COMMENT_HEAD."</td><td align='left'><textarea name='icomment' cols='30' rows='2' class='option_text' onfocus='o_submit_single = false;' onblur='o_submit_single = true;' style='resize:vertical;'>".stripslashes($default_text)."</textarea></td></tr>"; 
    } 
    ...
    if ($this->has_text_check_type) {
      ...
      $formString .= "<select name='ictype' style='margin-left:0;padding-left:0;'>"; 
    ...
    if ($this->hasRequire){
      ...
      if (!isset($item_value['require'])) {
        $formString .= "<input style='padding-left:0;margin-left:0;' type='radio' name='require' value='1' checked>".TEXT_MUSTBE_INPUT; 
        ...
        if ($item_value['require'] == '1') {
          ...
          $formString .= "<input style='padding-left:0;margin-left:0;' type='radio' name='require' value='1'>".TEXT_MUSTBE_INPUT; 
    ...
    if ($this->has_textarea_default) {
      ...
      $formString .= "<tr><td width='25%' align='left'>".TEXT_ITEM_TEXTAREA_HEAD."</td><td align='left'><textarea name='itextarea' cols='30' rows='10' class='option_text' onfocus='o_submit_single = false;' onblur='o_submit_single = true;' style='resize:vertical;'>".stripslashes($default_text)."</textarea></td></tr>"; 
    }
  
    if ($this->hasSelect) {
      if ($this->hasComment) {
        $formString .= "<tr><td width='25%' align='left'>".TEXT_ITEM_SELECT_COMMENT."</td><td align='left'><input type='text' name='secomment' value='".stripslashes($item_value['secomment'])."' class='option_text'></td></tr>"; 
      }
      if ($this->has_select_default) {
        $formString .= "<tr><td width='25%' align='left'>".TEXT_ITEM_SELECT_DEFAULT."</td><td align='left'><input type='text' name='sedefault' value='".stripslashes($item_value['sedefault'])."' class='option_text'></td></tr>"; 
      }
    ...
    if ($this->has_radio_comment) {
      $formString .= "<tr><td width='25%' align='left'>".TEXT_ITEM_TEXT_COMMENT_HEAD."</td><td align='left'><input type='text' name='racomment' value='".stripslashes($item_value['racomment'])."' class='option_text'><br><span id='rname_error' style='color:#ff0000;'></span></td></tr>"; 
    }
    
    if ($this->has_default) {
      $formString .= "<tr><td width='25%' align='left'>".TEXT_ITEM_RADIO_DEFAULT_SELECT."</td><td align='left'><input type='text' name='default_radio' value='".stripslashes($item_value['default_radio'])."' class='option_text'></td></tr>"; 
    }
      ...
      if (!isset($item_value['radio_image'])) {
        ...
        if (!empty($item_value['radio_image'])) {
          ...
          foreach ($item_value['radio_image'] as $ri_key => $ri_value) {
            $formString .= "<tr><td align='left'>".TEXT_ITEM_SELECT_HEAD."</td><td align='left'><textarea name='ro_".$i."' onfocus='o_submit_single = false;' onblur='o_submit_single = true;' cols='30' rows='5' class='option_text' style='resize:vertical;'>".stripslashes($ri_value['title'])."</textarea>&nbsp;<a href=\"javascript:void(0);\" onclick=\"delete_radio(this, ".$i.");\">".tep_html_element_button(TEXT_ITEM_DEL_LINK)."</a></td></tr>"; 
