...
foreach($_SESSION['options'] as $op_value){
  $address_query = tep_db_query("select * from ". TABLE_COUNTRY_AREA ." where name='". $op_value[1] ."'");
  $address_num = tep_db_num_rows($address_query);
  
  $country_query = tep_db_query("select * from ". TABLE_COUNTRY_FEE ." where name='". $op_value[1] ."'");
