...
for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
  ...
  echo '          <tr>' . "\n" .
         ...
         class="confirmation_product_num_info">' .  $order->products[$i]['qty'] .
         '&nbsp;'. NUM_UNIT_TEXT.  (!empty($product_info['products_exchange_rate']) && tep_get_full_count_in_order2($order->products[$i]['qty'], (int)$order->products[$i]['id']) ? '<br><span style="font-size:10px">'.  tep_get_full_count_in_order2($order->products[$i]['qty'], (int)$order->products[$i]['id']) .'</span>': '') . '</td>' . "\n" .
