...
// Quantity box or information as an input box or text
    ...
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      $product_info['products_small_sum'] = $product_info['price_type'] == 1 ?  $product_info['products_small_sum'] : '';
      ...
      if(!in_array($products[$i]['id'],$check_products_option)){
        echo '<a style="display:block; margin-left:4px;" onclick="change_num(\''.$p_id.'\',\'up\',1,'. $p_a_quan.', \''.$products[$i]['quantity'].'\', \''.$origin_small.'\')"><img src="images/ico/nup.gif" alt="up" style="vertical-align:bottom;"></a>';
        echo '<a onclick="change_num(\''.$p_id.'\',\'down\',1,'. $p_a_quan.', \''.$products[$i]['quantity'].'\', \''.$origin_small.'\')" style="display:block"><img src="images/ico/ndown.gif" alt="down" style="vertical-align:top;"></a>';
      }else{

        echo '<img src="images/ico/nup.gif" alt="up" style="margin-left:4px;vertical-align:bottom;">';
        echo '<img src="images/ico/ndown.gif" alt="down" style="margin-left:3px;vertical-align:top;">';
      }
      echo '</div>'; 
      echo '<div style="float:left;margin:5px auto"><font style="font-size:14px">'.NUM_UNIT_SYMBOL.'</font></div>';
           echo '<div style="clear:both; margin-left:10px;">'.
             (!empty($product_info['products_exchange_rate']) &&
              tep_get_full_count_in_order2($products[$i]['quantity'],
                $products[$i]['id']) ? '<span id="one_price_show_'.$products[$i]['id'].'" style="font-size:14px">'.  tep_get_full_count_in_order2($products[$i]['quantity'], $products[$i]['id']) .'</span>': '') . "\n";
    
            echo  (!empty($product_info['products_exchange_rate']) && tep_get_full_count_in_order2($products[$i]['quantity'], $products[$i]['id']) ? '<span id="one_price_'.$products[$i]['id'].'" style="display:none;">'.  tep_get_full_count_in_order2($products[$i]['quantity'], $products[$i]['id'],true) .'</span>': '') . "\n"."</div></td>";
...
// Product name, with or without link
    ...
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      ...
      if ($cart_pro_status['products_status'] == 0  || $cart_pro_status['products_status'] == 3) {
        ...
        echo '    <td class="main" >'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' .  tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .  (int)$products[$i]['id']) . '">' . $products[$i]['model'] .  '</a><br>':'').'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . (int)$products[$i]['id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
      }
      //echo '    <td class="main"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
    } else {
      echo '    <td class="main">'.(((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART))?'<a href="' .  tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . (int)$products[$i]['id']) . '">' . $products[$i]['model'] . '</a><br>':'').'<b>' . $products[$i]['name'] . '</b>';
...
// Product price  
    if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
      echo '    <td align="right" class="main" style="width:60px;"><br><span id="pri_'.$products[$i]['id'] .'">';
