...
// $raw_date needs to be in this format: YYYY-MM-DD HH:MM:SS
  function tep_date_long($raw_date) {
    ...
    $returntime = date("Y年m月d日l", mktime($hour,$minute,$second,$month,$day,$year));
  ...
  function page_head(){
    ...
    switch (str_replace('/', '', $_SERVER['SCRIPT_NAME'])) {
      ...
      case FILENAME_PRESENT:
        ...
        if ($present['text'])
          $description = mb_substr(strip_tags($present['text']),0,200);
        break;
      case FILENAME_SPECIALS:
        ...
        /*
        $title = HEADING_TITLE . ' ゲームマネー';
        $keywords="RMT,激安,安い,特価,販売,買取,MMORPG,アイテム,アカウント,ゲーム通貨";
        $description="今日のお買い得ゲーム一覧。RMTのことならRMTゲームマネーへ";
        */
        break;
      ...
      case FILENAME_MANUFACTURERS:
        ...
        $description  = "ゲームメーカーの一覧です。スクウェア・エニックス、NCJ、ガンホーなど。RMTのことならRMTゲームマネーへ";
        */
        break;
