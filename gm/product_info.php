    ...
    ?>
      ...
      while($sub_colors = tep_db_fetch_array($sub_colors_query)) {
        ...
        if($mcnt == 1) {
          ?>
            <noscript>
            <?php echo '<a href="' . tep_href_link(DIR_WS_IMAGES . 'products/' . $product_info['products_image']) . '" rel="lightbox[products]">' .  tep_image3(DIR_WS_IMAGES . 'products/' . $product_info['products_image'], $product_info['products_name'], PRODUCT_INFO_SMALL_IMAGE_WIDTH, PRODUCT_INFO_SMALL_IMAGE_HEIGHT, 'hspace="2" vspace="2" class="image_border"') . '</a><br>'; ?>
            </noscript>
            <?php
        }
        ?>
