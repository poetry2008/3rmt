...
<!-- left_navigation_eof //-->
      ...
      <!-- body_text //--> 
      <?php
if (!isset($_GET['tags_id'])) $_GET['tags_id']= NULL;
if (!isset($_GET['action'])) $_GET['action']= NULL;
  if ($category_depth == 'nested') {
    $category_query = tep_db_query("select cd.categories_name, c.categories_image from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . $current_category_id . "' and site_id = '" .  SITE_ID . "' and cd.categories_id = '" . $current_category_id . "' and cd.language_id = '" . $languages_id . "' and cd.site_id = '".SITE_ID."'");
    $category = tep_db_fetch_array($category_query);
?> 
<div id="content">
      <?php  
...
// check to see if there are deeper categories within the current category
      ...
      for($i=0, $n=sizeof($category_links); $i<$n; $i++) {
        $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . $category_links[$i] . "' and site_id = '" . SITE_ID . "' and c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' and cd.site_id =  '".SITE_ID."' order by sort_order, cd.categories_name");
      ...
      $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . $current_category_id . "' and site_id = '" . SITE_ID . "' and c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' and cd.site_id = '".SITE_ID."' order by sort_order, cd.categories_name");
    }
...
// create column list
  ...
  {
    $products_query = "select *, p.products_id , IF(s.status, s.specials_new_products_price, p.products_price) as final_price from " .  TABLE_PRODUCTS_TO_TAGS . " as p2t join ". TABLE_PRODUCTS . " as p on p2t.products_id = p.products_id left join " . TABLE_PRODUCTS_DESCRIPTION . " as pd on p.products_id = pd.products_id left join " . TABLE_SPECIALS . " as s on p.products_id = s.products_id where p2t.tags_id = " .  intval($_GET['tags_id']) . " and pd.site_id = '".SITE_ID."'";
  } else {
    $products_query = "select *, p.products_id, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from " .  TABLE_PRODUCTS_TO_TAGS . " as p2t join ". TABLE_PRODUCTS . " as p on p2t.products_id = p.products_id left join " . TABLE_PRODUCTS_DESCRIPTION . " as pd on p.products_id = pd.products_id left join " . TABLE_SPECIALS . " as s on p.products_id = s.products_id where p2t.tags_id = " .  intval($_GET['tags_id']) . " and pd.site_id = '".SITE_ID."'";
  } 
...
<?php include(DIR_WS_MODULES . FILENAME_PRODUCT_LISTING); ?>
  ...
  // Add Color =============================================================================
	...
	//======================================================================================
  // maker
  } elseif($_GET['action'] && $_GET['action'] == 'select' && 0) { // select my game 
?> 
