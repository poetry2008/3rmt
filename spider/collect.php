...
$game_type=$game_type;
  ...
  $collect_site_url = array('rmt.kakaran.jp');
  ...
  if(strpos($url_array[$site_value],'rmt.kakaran.jp'){
    ...
    if($result_array[0]['url']){
      ...
      foreach($result_array[0]['url'] as $key=>$url){
         ...
         $collect_url_arr[$index][] = "rmt.kakaran.jp".$url;
...
if(strpos($result_array['inventory'][$product_key],'a')){
    $inventory_array[0]=0;
	 ...
	 case 'blade':
         if($category_value == 'buy'){
              ...
              $price = $result_array['prices'][$product_key]; 
              $result_str = $price;
            if($inventory_array[0] != ''){
	...
	case 'FNO':
        ...
        break; 
	case 'SUN':
              preg_match_all("|.*?([0-9,]+).*?口.*?|",$result_array['inventory'][$product_key],$temp_array);
              $price = $result_array['price'][$product_key]; 
              $result_str = $price*10;
              if($inventory_array[0] != ''){
	 ...
	 if($category_value == 'buy'){
          ...
          break;
            ...
            case 'latale':
             ...
             if($inventory_array[0] != ''){
                $price = $result_array['prices'][$product_key]*10; 
                $result_inventory = str_replace(',','',$result_array['inventory'][$product_key]); 
                $result_inventory = $result_inventory/10;
              }else{
                $price = $result_array['prices'][$product_key]*10; 
