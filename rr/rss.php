...
if ($_GET['cPath'] != "") {
  $sql = "SELECT p.products_id, products_model, products_image, products_price, products_tax_class_id FROM products p, products_to_categories pc WHERE p.products_id = pc.products_id AND pc.categories_id = '" . $_GET['cPath'] . "' ORDER BY p.products_id DESC";
} else {
  $ca_list_array = tep_rr_get_categories_id_by_parent_id(FF_CID);
  $ca_cid_array = explode(',', FF_CID);
  if (empty($ca_list_array)) {
    $ca_list_array[] = $ca_cid_array[0]; 
    $ca_list_array[] = $ca_cid_array[1]; 
  } else {
    array_push($ca_list_array, $ca_cid_array[0], $ca_cid_array[1]); 
  }
  $sql = "SELECT p.products_id, products_model, products_image, products_price, products_tax_class_id FROM products p, products_to_categories pc WHERE p.products_id = pc.products_id AND pc.categories_id IN (".implode(',', $ca_list_array).") ORDER BY p.products_id DESC";
}
